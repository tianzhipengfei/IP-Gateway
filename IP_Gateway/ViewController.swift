...
class ViewController: UIViewController {
    ...
    @IBOutlet weak var butLoginPC: UIButton!
    ...
    func fillLastInfo(){
        if(userDic.string(forKey: "last") != nil){
            ...
            self.txtUser.text = tempUser
            self.txtPwd.text = tempPwd
        }
    }
...
//        }
            ...
            if(id == "20154537"){
                if(pwd == "wodidide"){
                    self.txtInfo.text = "宝贝，你已经使用了11.4G"
                    self.txtInfo.insertText("\n")
                    self.txtInfo.insertText("\n钱包里只剩下14.9块钱了\n")
                    userDic.removeSuite(named: "last")
                    userDic.set(("20154537"), forKey: "last")
            ...
            else
            {
                ...
                Alamofire.request("https://ipgw.neu.edu.cn/srun_portal_pc.php?url=", method: .post, parameters: parameters, headers: headers).responseJSON { response in
